...
export function genSidersMenus(routes: RouteRecordRaw[]): MenuOption[] {
  return routes.map((route) => {
    const menu = {
      label: route.name,
      key: route.name,
...
export function generateRoutes(routes: RouteRecordRaw[]): RouteRecordRaw[] {
