...
// 获取 hosts 统计信息
const getHostsStats = async () => {
  loading.value = true
  try {
    const res = await getAllHosts({ page: 1, pageSize: 100 })
    if (res.code === 200) {
      const hosts = res.data.list
      hostsStats.value = {
        total: hosts.length,
        active: hosts.filter(h => h.status === 0).length,
        readonly: hosts.filter(h => h.is_readonly === 1).length
      }
      // 获取最近的3个hosts文件
...
const navigateToHosts = (hostId: number) => {
  ...
  <div class="home-container">
    ...
    <div class="welcome-section">
      <div class="welcome-content">
        <SvgIcon name="logo" size="120px" />
        <h1 class="welcome-title">Switch Hosts R</h1>
        <p class="welcome-subtitle">简单易用的 hosts 文件管理工具</p>
        ...
        <div class="stats-grid">
          <n-card class="stat-card" hoverable>
            <div class="stat-content">
              <div class="stat-number">{{ hostsStats.total }}</div>
          ...
          <n-card class="stat-card" hoverable>
            <div class="stat-content">
              <div class="stat-number">{{ hostsStats.active }}</div>
          ...
          <n-card class="stat-card" hoverable>
            <div class="stat-content">
              <div class="stat-number">{{ hostsStats.readonly }}</div>
              <div class="stat-label">只读模式</div>
            </div>
            <div class="stat-icon">
        ...
        >
          <div class="recent-content">
            <div class="recent-info">
              <h3 class="recent-name">{{ host.name }}</h3>
              <p class="recent-path">{{ host.hosts_path }}</p>
              <div class="recent-meta">
                <n-tag v-if="host.is_readonly" type="warning" size="small">只读</n-tag>
                <n-tag v-if="host.status === 0" type="success" size="small">已启用</n-tag>
